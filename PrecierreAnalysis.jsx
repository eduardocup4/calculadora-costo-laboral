...
import { UserPlus, UserMinus, AlertTriangle, Download, ArrowRight, TrendingUp, TrendingDown, FileSpreadsheet, FileText, Info } from 'lucide-react';
...
const PrecierreAnalysis = ({ analysis, periodsData, onNewAnalysis }) => {
  const { generalDiff, alerts, variaciones, altas, bajas } = analysis;
  ...
  const handleDownloadPDF = () => {
      const headers = ['Nombre', 'Concepto', 'Anterior', 'Actual', 'Diferencia', 'Variación %'];
    ...
    <div className="animate-fade-in space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center divide-y md:divide-y-0 md:divide-x divide-slate-200">
          ...
          <div className="py-4 md:py-0">
            ...
            <div className={`flex items-center justify-center gap-2 text-3xl font-bold ${
              generalDiff.absolute > 0 ? 'text-red-600' : 'text-emerald-600'
            }`}>
        ...
        <div className="bg-white rounded-2xl border-2 border-amber-200 p-6 shadow-lg">
          <div className="flex items-center justify-between mb-6">
            ...
            <select
              ...
              className="px-4 py-2 border-2 border-amber-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-amber-500 outline-none"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-white p-6 rounded-2xl border-2 border-emerald-200 shadow-lg">
          ...
          {altas.length > 0 ? (
            ...
            <div className="py-12 text-center">
              <Info className="w-12 h-12 text-emerald-300 mx-auto mb-3" />
              <p className="text-sm text-emerald-600">No se registraron nuevos ingresos</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-gradient-to-br from-red-50 to-white p-6 rounded-2xl border-2 border-red-200 shadow-lg">
          ...
          {bajas.length > 0 ? (
            ...
            <div className="py-12 text-center">
              <Info className="w-12 h-12 text-red-300 mx-auto mb-3" />
              <p className="text-sm text-red-600">No se detectaron bajas</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-lg overflow-hidden">
        <div className="p-6 border-b-2 border-slate-200 bg-slate-50">
          <h3 className="font-bold text-xl text-slate-800">Variaciones Salariales Específicas ({variaciones.length})</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {variaciones.length > 0 ? variaciones.map((v, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-medium text-slate-900">{v.nombre}</td>
                  <td className="px-6 py-4 text-slate-600">{v.concepto}</td>
                  ...
                  <td className={`px-6 py-4 text-right font-bold ${
                    v.diff > 0 ? 'text-red-600' : v.diff < 0 ? 'text-emerald-600' : 'text-slate-500'
                  }`}>
                    {v.diff > 0 ? '+' : ''}{formatCurrency(v.diff)}
                  </td>
                  <td className={`px-6 py-4 text-right font-bold ${
                    v.pct > 0 ? 'text-red-600' : v.pct < 0 ? 'text-emerald-600' : 'text-slate-500'
                  }`}>
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center">
                    ...
                    <p className="text-slate-400">No se detectaron variaciones significativas</p>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {/* Botón de Nueva Análisis */}
      <div className="flex justify-center pt-8">
        <button 
          onClick={onNewAnalysis} 
          className="flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-slate-800 to-slate-900 text-white rounded-2xl hover:shadow-xl transition-all font-bold text-lg"
        >
